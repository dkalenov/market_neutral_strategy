...
def run_pairs_backtest_with_backtestingpy(input_csv, signals_csv):
    ...
    def engine_step(self, idx_bar):
        ...
        for next_idx_sig, sigrow in todays:
            ...
            # === COMPUTE CURRENT Z-SCORE (INTERNAL) ===
            start_idx_z = max(0, idx_bar - WINDOW + 1)
            if (idx_bar - start_idx_z + 1) < 10:
                continue

            log_a = np.log(close[a].iloc[start_idx_z:idx_bar + 1].values)
            log_b = np.log(close[b].iloc[start_idx_z:idx_bar + 1].values)

            spread = pd.Series(log_a - hedge * log_b)
            ...
            if not (pd.isna(sigrow.get("dollar1")) or pd.isna(sigrow.get("dollar2"))):
                ...
                alloc_a, alloc_b = vol_parity_notional_from_series(
                    s1_hist, s2_hist, hedge, cap_pair_usd=caps
                )
